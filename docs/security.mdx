---
sidebar_position: 40
sidebar_label: Sign in security
...
<Tabs>
  <TabItem value="debug-provider" label="Debug provider">
Provide the [debug provider](https://developers.google.com/identity/sign-in/ios/appcheck/debug-provider) with the API key:

```ts
await GoogleOneTapSignIn.enableAppCheck({
  debugProviderWithAPIKey: config.apiKey,
});
```
...
```ts
await GoogleOneTapSignIn.enableAppCheck({
  debugProviderWithAPIKey: undefined, // or don't provide this parameter at all
});
```
