...
Call [`GoogleOneTapSignIn.authenticate`](../one-tap#authenticate) when the page loads. A successful button sign-in is delivered to the same `onResponse` callback.
