---
sidebar_position: 5
sidebar_class_name: sponsor-heart
description: 'Web setup guide. Works with NextJS, Vite, and other web frameworks. Covers Google client ID configuration and script loading.'
---

# Web setup guide

The web implementation doesn't depend on React Native (or React Native Web). That means you can use it even with regular web apps created with NextJS, Vite and etc.

import Banner from '../_sponsorBanner.mdx';

<Banner />

On the web, you need to load the Google Client Library and make it available in the browser **before** calling any of the APIs exposed by this package.

There are different ways to load the client script. Some of them are:

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs>
  <TabItem value="Next.js" label="Next.js">

```tsx
import Script from 'next/script';

<Script
  src="https://accounts.google.com/gsi/client"
  strategy="lazyOnload"
  onLoad={() => {
    // present the sign in popup
  }}
/>;
```

  </TabItem>

  <TabItem value="html" label="Simple html">

[See here](https://developers.google.com/identity/gsi/web/guides/client-library)

  </TabItem>

  <TabItem value="useEffect" label="useEffect">

```tsx
useEffect(() => {
  const scriptTag = document.createElement('script');
  scriptTag.src = 'https://accounts.google.com/gsi/client';
  scriptTag.async = true;
  scriptTag.onload = () => {
    setLoaded(true);
  };
  scriptTag.onerror = () => {
    console.error('Failed to load Google script');
  };

  document.body.appendChild(scriptTag);
}, []);
```

  </TabItem>

</Tabs>

After the script is loaded, you can call `GoogleOneTapSignIn.authenticate` and render the [`WebGoogleSigninButton`](/docs/buttons/web).

If you call `authenticate` before the client library is loaded, the [`onResponse` callback](/docs/api#webonetapauthenticatecallbacks) receives an `error` with the [`PLAY_SERVICES_NOT_AVAILABLE` error code](../errors#status-codes). Advanced web authentication methods report the same error through [`onError`](/docs/api#webonetapsignincallbacks).

You can read the official docs for loading the Client Library [here](https://developers.google.com/identity/gsi/web/guides/client-library).

## Usage

See [here](../web-support#usage).
