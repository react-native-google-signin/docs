...
On Android, presence of up-to-date Google Play Services is required to show the authentication and authorization modals. It is therefore necessary to call `checkPlayServices` any time prior to calling the authentication / authorization methods and only call those if `checkPlayServices` is successful.
