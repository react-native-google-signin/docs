...
This module is being actively developed, and new features are added regularly. Since the first release in July 2023, there has been more than 1 release per month on average (last updated in September 2024).
:::
...
It is mandatory to call this method before attempting to call any of the sign-in methods. This method is synchronous meaning you can call e.g. `signIn` right after it. Typically, you would call `configure` only once, soon after your app starts.
...
The underlying Android SDK separates authentication and authorization - that means that on Android you can request an access token and call Google APIs on behalf of the user without signing the user in before that.
